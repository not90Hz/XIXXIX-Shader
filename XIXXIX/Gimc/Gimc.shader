Shader "XIXXIX/Gimc"
{
	Properties
	{
		...
		_ScanlineFreq("Scanline Freq", Range(0 , 50)) = 3
		_ScanlineThickness("Scanline Thickness", Range(0 , 1)) = 0.5
		_ScanlineRotation("Scanline Rotation", Float) = 0
		...
		SubShader
		{
			...
			uniform float4 _ScanlineTex_ST;
			uniform float _ScanlineRotation;
			...
			void surf(Input i , inout SurfaceOutputCustomLightingCustom o)
			{
				...
				float2 uv_ScanlineTex = i.uv_texcoord * _ScanlineTex_ST.xy + _ScanlineTex_ST.zw;
				float3 ase_worldPos = i.worldPos;
				float cos10_g12 = cos(radians((_ScanlineRotation + 90.0)));
				float sin10_g12 = sin(radians((_ScanlineRotation + 90.0)));
				float2 rotator10_g12 = mul(ase_worldPos.xy - float2(0.5,0.5) , float2x2(cos10_g12 , -sin10_g12 , sin10_g12 , cos10_g12)) + float2(0.5,0.5);
				float2 appendResult8_g12 = (float2(_ScanlineFreq , 1.0));
				float mulTime257 = _Time.y * _ScanlineSpeed;
				float2 appendResult9_g12 = (float2(mulTime257 , 0.0));
				float2 appendResult10_g13 = (float2(_ScanlineThickness , 1.0));
				float2 temp_output_11_0_g13 = (abs((frac((rotator10_g12 * appendResult8_g12 + appendResult9_g12)) * 2.0 + -1.0)) - appendResult10_g13);
				float2 break16_g13 = (1.0 - (temp_output_11_0_g13 / fwidth(temp_output_11_0_g13)));
				float4 lerpResult277 = lerp(float4(0,0,0,0) , (_ScanlineColor * tex2D(_ScanlineTex, uv_ScanlineTex)) , saturate(min(break16_g13.x , break16_g13.y)));
				float4 lerpResult269 = lerp(temp_cast_2 , lerpResult277 , _LerpScanline);
				float4 Scanline272 = lerpResult269;
				float4 Emissionzsm209 = (EmissionTex242 + RimLight208 + Scanline272);
